...
function CalendarPage() {
  ...
  return (
    <div className="page-container wm-page">
      ...
      <div className="wm-calendar-shell">
                    ...
                    <div key={dayKey} className={'wm-calendar-grid__cell' + (isCurrentMonth ? '' : ' is-muted')}>
                      ...
                      <div className="wm-calendar-grid__events">
                        {dayItems.map(function (item) {
                          return (
                            <div key={item.id} className="wm-calendar-event" style={{ '--event-accent': getEventTone(item.type) }}>
                        ...
                        {visibleItems.filter(function (item) {
                          return new Date(item.start).toISOString().slice(0, 10) === dayKey;
                        }).length > 3 ? <small>More events</small> : null}
                      </div>
                    </div>
                  );
                })}
                  ...
                  return (
                    <div key={dayKey} className="wm-agenda-column">
                      ...
                      {dayItems.length === 0 ? (
                        ...
                        dayItems.map(function (item) {
                          return (
                            <div key={item.id} className="wm-agenda-item" style={{ '--event-accent': getEventTone(item.type) }}>
